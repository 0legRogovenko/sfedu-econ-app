...
class FakeContactsApi implements ContactsApi {
  ...
  @override
  Future<List<Map<String, dynamic>>?> fetchContacts() async => responses[_i++];
}
...
void main() {
  ...
  test('refresh при ошибке сети: кэш цел, offline=true', () async {
    ...
    final feed = await ContactsRepository(
      FakeContactsApi([null]),
      db,
    ).refresh();
  ...
  test('refresh заменяет кэш целиком (удалённые контакты исчезают)', () async {
    final repo1 = ContactsRepository(
      FakeContactsApi([
        [_json(1), _json(2)],
      ]),
      db,
    ...
    final repo2 = ContactsRepository(
      FakeContactsApi([
        [_json(2)],
      ]),
      db,
  ...
  test('loadCached отдаёт сохранённое', () async {
    ...
    final cached = await ContactsRepository(
      FakeContactsApi([null]),
      db,
    ).loadCached();
