...
void main() {
  test(
    'мягкая миграция: текущая группа становится единственной избранной',
    () async {
  ...
  test(
    'сохранённый список читается как есть, миграция не срабатывает',
    () async {
      final container = await _container(
        prefsValues: {
          'selected_group_id': 3,
  ...
  test(
    'add добавляет в конец и пишет в prefs, дубликаты игнорируются',
    () async {
      final container = await _container(prefsValues: {'selected_group_id': 3});
  ...
  test(
    'удаление активной избранной переключает активную на первую оставшуюся',
    () async {
  ...
  test(
    'удаление единственной избранной оставляет активную группу как есть',
    () async {
      final container = await _container(prefsValues: {'selected_group_id': 3});
  ...
  test(
    'устаревшие id заменяются реальной группой после загрузки справочника',
    () async {
      final container = await _container(
        prefsValues: {
      ...
      await container.read(favoriteGroupIdsProvider.notifier).reconcile([
        6,
        13,
      ]);
