...
@pytest.mark.parametrize(
    ...
    [
        ...
        "1040-1125\n1130-1215",  # вторая половина не из сетки (1130-1215) и span < блока
        "1042-1125\n1155-1240",  # первая половина не из сетки (1042≠1040) — артефакт
    ],
)
def test_time_outside_the_pair_grid_is_not_guessed(time_raw):
    ...
    половины, битые артефакты извлечения и склейка всей колонки времени
    в одну строку.
    """
...
@pytest.mark.parametrize(
    "time_raw, pair",
    [
        ("1040-1125\n1155- 1240", 2),  # понедельник 13471: языковой блок 10:40–12:40
        ("1245- 1330\n1345-1430", 3),  # 12:45–14:30
        ("1435-1520\n1550-1635", 4),  # 14:35–16:35
        ("1640-1725\n1735-1820", 5),  # между 5 и 6 — раньше отвергалось
        ("1825-1910\n1915-2000", 6),  # между 6 и 7 — раньше отвергалось
    ],
)
def test_straddle_block_maps_to_the_starting_pair(time_raw, pair):
    """Сдвинутый блок в 2 ак. часа: вторая половина пары N + первая N+1.

    Раньше такие ячейки отвергались как «съехавшее окно» — и весь языковой
    понедельник 3 курса (13471 p2, все группы) уходил в UnparsedCell: 27 ячеек,
    у студентов пустой день. Живой файл доказал, что это легитимная нотация.
    Это не откат запрета на угадывание: обе половины обязаны ДОСЛОВНО совпадать
    с сеткой и быть соседними, а UI показывает реальное время (10:40–12:40)
    из parse_time_bounds, так что студент ко времени не промахнётся.
    """
    assert parse_pair_number(time_raw) == pair
