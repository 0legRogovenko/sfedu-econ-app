...
def parse_pair_number(time_raw: str) -> int | None:
    ...
    ('800-935'). Если не совпало — это может быть блок в 3+ ак. часа (иняз по
    вторникам, вечерние окна магистров): тогда пара по времени начала. Всё
    прочее — одинокие половины, съехавшие мимо границы окна, битые артефакты —
    честный None: пара из них не выводится.
    """
